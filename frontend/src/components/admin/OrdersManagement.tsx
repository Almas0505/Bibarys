...
export default function OrdersManagement() {
  ...
  const statusOptions = [
    ...
    { value: 'cancelled', label: 'Болдырмалынды' },
  ];
  ...
  const loadOrders = async () => {
    try {
      ...
      setError(err.response?.data?.detail || 'Тапсырыстарды жүктеу кезінде қате орын алды');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleUpdateStatus = async () => {
    ...
    try {
      ...
      alert('Тапсырыс статусы жаңартылды!');
      setShowStatusModal(false);
      setSelectedOrder(null);
      loadOrders();
    } catch (err: any) {
      console.error('Error updating status:', err);
      alert(err.response?.data?.detail || 'Статусты жаңарту кезінде қате орын алды');
    }
  };
    ...
    <div>
      <div className="bg-white rounded-lg shadow-md p-6">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-xl font-bold">Тапсырыстарды басқару</h3>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          <Input
            type="text"
            placeholder="Нөмірі немесе email бойынша іздеу..."
            value={searchQuery}
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b">
                <th className="text-left py-2 px-4">№ Тапсырыс</th>
                ...
                <th className="text-left py-2 px-4">Күні</th>
                ...
                <tr key={order.id} className="border-b hover:bg-gray-50">
                  ...
                  <td className="py-3 px-4">
                    <div className="flex gap-2">
                      ...
                      >
                        {order.status === 'pending' || order.status === 'processing' ? 'Статусты өзгерту' : 'Көру'}
                      </button>
                    </div>
                  </td>
                </tr>
      ...
      <Modal
        ...
        title={`${selectedOrder?.status === 'delivered' || selectedOrder?.status === 'cancelled' ? 'Тапсырыс мәліметтері' : 'Тапсырыс статусын өзгерту'} #${selectedOrder?.id}`}
      >
        <div className="space-y-4">
          ...
          {selectedOrder && (
            <div className="bg-gray-50 p-4 rounded-lg space-y-2 mb-4">
              ...
              <p className="text-sm"><span className="font-semibold">Сомасы:</span> {formatPrice(selectedOrder.total_price)}</p>
              <p className="text-sm"><span className="font-semibold">Күні:</span> {formatDateTime(selectedOrder.created_at)}</p>
              <p className="text-sm">
            ...
            <>
              <div>
                ...
                >
                  ...
                  <option value="cancelled">Болдырмалынды</option>
                </select>
              </div>
