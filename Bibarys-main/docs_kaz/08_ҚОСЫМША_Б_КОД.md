...
Барлық SQLAlchemy модельдері:
- `User` - Пайдаланушылар
- `Category` - Тауарлар санаттары
- `Product` - Тауарлар
- `CartItem` - Сатып алу себеті
- `Order` - Тапсырыстар
- `OrderItem` - Тапсырыс позициялары
- `Review` - Пікірлер
- `Wallet` - Виртуалды әмияндар
- `WalletTransaction` - Әмиян транзакциялары
- `WishlistItem` - Таңдаулылар тізімі
...
- `GET /orders/my` - Пайдаланушының тапсырыстары
- `GET /orders/{order_id}` - Тапсырысты ID бойынша алу
- `PUT /orders/{order_id}/status` - Тапсырыс мәртебесін өзгерту (Seller/Admin)
- `DELETE /orders/{order_id}/cancel` - Тапсырысты болдыру (Customer)
