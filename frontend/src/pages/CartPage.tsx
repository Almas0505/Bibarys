...
export default function CartPage() {
  ...
  const handleRemove = async (itemId: number) => {
    if (confirm('Тауарды себеттен өшіру керек пе?')) {
      await dispatch(removeFromCart(itemId));
    }
  };

  const handleClearCart = async () => {
    if (confirm('Себетті толығымен тазарту керек пе?')) {
  ...
  if (isLoading) {
    return <LoadingSpinner text="Себет жүктелуде..." />;
  }

  if (cart.items.length === 0) {
    return (
      <div className="container mx-auto px-4 py-16 text-center">
        <h2 className="text-3xl font-bold text-gray-800 mb-4">Себет бос</h2>
        <p className="text-gray-600 mb-8">Сатып алуды жалғастыру үшін себетке тауарлар қосыңыз</p>
        <Link
          to="/shop"
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-3xl font-bold">Себет</h1>
        <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1">
          <div className="bg-white rounded-lg shadow-md p-6 sticky top-4">
            <h2 className="text-2xl font-bold mb-6">Жиыны</h2>

            <div className="space-y-4 mb-6">
              <div className="flex items-center justify-between text-gray-600">
                <span>Тауарлар:</span>
                <span>{cart.total_items} дана</span>
              </div>
              <div className="flex items-center justify-between text-gray-600">
                <span>Сомасы:</span>
