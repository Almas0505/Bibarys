...
export default function OrderDetailsPage() {
  ...
  const handleCancelOrder = async () => {
    if (order) {
      try {
        ...
        alert('Тапсырыс сәтті қабылданбады');
        setShowCancelModal(false);
      } catch (error: any) {
        alert(error.message || 'Тапсырысты болдырмау кезінде қате орын алды');
      }
    }
  };
  ...
  const handleRepeatOrder = async () => {
    ...
    try {
      ...
      alert('Тауарлар себетке қосылды');
      navigate('/cart');
    } catch (error: any) {
      alert(error.message || 'Тауарларды себетке қосу кезінде қате орын алды');
    }
  };

  if (isLoading) {
    return <LoadingSpinner text="Тапсырыс жүктелуде..." />;
  }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold mb-2">Тапсырыс #{order.id}</h1>
          <p className="text-gray-600">{formatDateTime(order.created_at)}</p>
          <p className="text-xs text-gray-500 mt-1">
            Жаңартылды: {lastUpdated.toLocaleTimeString('kk-KZ')} • Әр 10 сек автоматты жаңарту
          </p>
        </div>
      ...
      {order.status !== 'cancelled' && (
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          <h2 className="text-xl font-bold mb-6">Тапсырыс мәртебесі</h2>
                ...
                <div key={status} className="flex items-center flex-1">
                  <div className="flex flex-col items-center">
                    ...
                    <span className="text-xs mt-2 text-center">
                      ...
                      {status === 'delivered' && 'Қабылданды/Жеткізілді'}
                    </span>
                  </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="bg-white rounded-lg shadow-md p-6">
            <h2 className="text-xl font-bold mb-4">Тауарлар</h2>
                ...
                <div key={item.id} className="flex items-center justify-between pb-4 border-b last:border-b-0">
                  <div>
                    <Link to={`/product/${item.product_id}`} className="font-semibold hover:text-primary-600">
                      Тауар #{item.product_id}
                    </Link>
                    <p className="text-gray-600 text-sm">Саны: {item.quantity} дана</p>
                  </div>
                  <div className="text-right">
                    <div className="font-bold">{formatPrice(item.price_at_purchase * item.quantity)}</div>
                    <div className="text-gray-600 text-sm">Данасына {formatPrice(item.price_at_purchase)}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          {/* Delivery Info */}
          <div className="bg-white rounded-lg shadow-md p-6">
            <h2 className="text-xl font-bold mb-4">Жеткізу туралы ақпарат</h2>
            <div className="space-y-3 text-sm">
              <div>
                <span className="text-gray-600">Жеткізу тәсілі:</span>
                <span className="ml-2 font-semibold">{order.delivery_method}</span>
              </div>
              <div>
                <span className="text-gray-600">Жеткізу құны:</span>
                <span className="ml-2 font-semibold">{formatPrice(order.delivery_cost)}</span>
              </div>
              <div>
                <span className="text-gray-600">Мекенжай:</span>
              ...
              {order.estimated_delivery && order.estimated_delivery !== '' && (
                <div>
                  <span className="text-gray-600">Күтілетін жеткізу күні:</span>
                  <span className="ml-2 font-semibold">{formatDateTime(order.estimated_delivery)}</span>
                </div>
              )}
              {order.notes && (
                <div>
                  <span className="text-gray-600">Түсініктеме:</span>
        ...
        <div className="lg:col-span-1">
          <div className="bg-white rounded-lg shadow-md p-6 sticky top-4">
            <h2 className="text-xl font-bold mb-4">Жиыны</h2>
      ...
      <Modal
        ...
        title="Тапсырысты болдырмау керек пе?"
      >
        <p className="mb-4">Бұл тапсырысты өшіргіңіз келетініне сенімдісіз бе?</p>
