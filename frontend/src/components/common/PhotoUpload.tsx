...
export const PhotoUpload: React.FC<PhotoUploadProps> = ({
  ...
  const uploadFiles = useCallback(async (files: FileList) => {
    ...
    try {
      ...
      // Загружаем файлы параллельно (все одновременно вместо очереди)
      const filesToUpload = Array.from(files).slice(0, maxFiles - uploadedImages.length);

      const uploadPromises = filesToUpload.map(async (file): Promise<UploadedImage | null> => {
        // Валидация типа файла
        if (!file.type.startsWith('image/')) {
          setError('Тек суреттерді жүктеуге болады');
          return null;
        }

        // Валидация размера (5MB)
        if (file.size > 5 * 1024 * 1024) {
          setError('Файл өлшемі 5MB-тан аспауы тиіс');
          return null;
        }
        ...
        try {
          ...
          setError('Суретті жүктеу кезінде қате орын алды');
          return null;
        }
      });
      ...
      >
        ...
        <div className="pointer-events-none">
          ...
          <p className="mt-2 text-sm text-gray-600">
            Суретті осында сүйреңіз немесе <span className="text-blue-600 font-medium">файлды таңдаңыз</span>
          </p>
          <p className="text-xs text-gray-500 mt-1">
            JPG, PNG немесе WebP 5MB-қа дейін (максимум {maxFiles} фото)
          </p>
        </div>
