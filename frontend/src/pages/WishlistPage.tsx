...
export default function WishlistPage() {
  ...
  const handleAddToCart = async (productId: number) => {
    try {
      ...
      alert(error || 'Себетке қосу кезінде қате орын алды');
    }
  };

  if (isLoading) {
    return <LoadingSpinner text="Таңдаулылар жүктелуде..." />;
  }

  if (wishlist.length === 0) {
    return (
      <div className="container mx-auto px-4 py-16 text-center">
        <h2 className="text-3xl font-bold text-gray-800 mb-4">Таңдаулылар тізімі бос</h2>
        <p className="text-gray-600 mb-8">Оларды жоғалтып алмау үшін тауарларды таңдаулыларға қосыңыз</p>
        <Link
          to="/shop"
          ...
          <div key={item.id} className="bg-white rounded-lg shadow-md overflow-hidden">
            <div className="relative">
              ...
              <button
                ...
                aria-label="Таңдаулылардан өшіру"
              >
